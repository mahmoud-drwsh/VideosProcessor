...
function Get-H265480EncodeArgs($inputPath, $outputPath, $metaTitle) {
    return "-y -hwaccel cuda -hwaccel_output_format cuda -i `"$inputPath`" -fps_mode passthrough -vf `"scale_cuda=-2:480:interp_algo=lanczos`" -map_metadata -1 -c:v hevc_nvenc -rc constqp -qp 25 -preset p7 -c:a copy -map_chapters 0 -metadata:s:a:0 title=`"$metaTitle`" `"$outputPath`""
}
