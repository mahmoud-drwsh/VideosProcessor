...
function Get-H265480EncodeArgs($inputPath, $outputPath, $metaTitle) {
    return "-y -vsync 0 -hwaccel cuda -hwaccel_output_format cuda -i `"$inputPath`" -vf `"scale_cuda=-2:480`" -map_metadata -1 -c:v hevc_nvenc -preset p7 -r 25 -c:a copy -map_chapters 0 -metadata:s:a:0 title=`"$metaTitle`" `"$outputPath`""
}
